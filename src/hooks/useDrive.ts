...
export function useDrive(slug: string) {
    ...
    }, [data?.value, sanitizedQuery])

    const sortedValue = useMemo(() => {
        return [...filteredValue].sort((a, b) => {
    ...
    }, [filteredValue, sortField, sortOrder])
    ...
    useEffect(() => {
        ...
        if (viewFileName) {
            const itemToPreview = data?.value.find((child) => child.name === viewFileName)
