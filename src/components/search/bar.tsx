'use client'
import { useTheme } from 'next-themes'
import React from 'react'
import { Button } from '@heroui/react'
import { ArrowLeft, SearchIcon, X } from 'lucide-react'

import { Navigation, User as UserType } from '@/app/login/types'
import AnimatedSearch from '@/ui/farmer/search'
import {
    Command,
    CommandInput,
    CommandItem,
    CommandList,
    CommandGroup,
    CommandEmpty,
} from '@/ui/search'
import { getSearchPlaceholder, capitalize } from '@/lib/utils'
import { Icon } from '@/lib'
import { Navigations, Actions } from '@/config/navigation'
import { Download } from '@/app/(space)/ui/download'
import { useSearchBar } from '@/zustand/store'

export function SearchBar({
    user,
    navigation: _navigation,
    children,
}: {
    user: UserType | null
    navigation: Navigation[]
    children: React.ReactNode
}) {
    const { setTheme } = useTheme()
    const ref = React.useRef<HTMLDivElement | null>(null)

    const {
        bounce,
        popPage,
        query,
        setQuery,
        pages,
        setPages,
        setDynamicNavigations,
        getNavigationGroups,
        isPageMode,
        open,
        setOpen,
        handleSelect,
    } = useSearchBar()

    const actions: Actions = {
        setTheme,
        setPages,
    }

    React.useEffect(() => {
        const navigations = Navigations(actions)

        setDynamicNavigations(navigations)
    }, [setDynamicNavigations])

    return (
        <AnimatedSearch open={open} setOpen={setOpen}>
            <Command
                ref={ref}
                className={`${open ? 'h-[80dvh] rounded-b-none md:h-[50dvh] md:rounded-lg md:border' : 'border'}`}
            >
                {open && (
                    <CommandList>
                        <CommandEmpty>No results found.</CommandEmpty>
                        {getNavigationGroups().map((group) => (
                            <CommandGroup key={group.heading} heading={capitalize(group.heading)}>
                                {group.navigationItems.map((n) => (
                                    <CommandItem
                                        key={n.id}
                                        description={n.description}
                                        href={n.href}
                                        startContent={<Icon icon={n.icon} />}
                                        title={n.title}
                                        value={n.title + n.description}
                                        onSelect={() => handleSelect(n)}
                                    />
                                ))}
                            </CommandGroup>
                        ))}
                    </CommandList>
                )}
                <CommandInput
                    classNames={{ base: `${open ? 'border-y md:border-t md:border-b-0' : ''}` }}
                    endContent={
                        <>
                            <Download />
                            {query ? (
                                <Button
                                    isIconOnly
                                    className='bg-default/20'
                                    radius='full'
                                    size='sm'
                                    startContent={<X size={18} />}
                                    variant='light'
                                    onPress={() => setQuery('')}
                                />
                            ) : (
                                children
                            )}
                        </>
                    }
                    placeholder={getSearchPlaceholder(user?.name)}
                    startContent={
                        <Button
                            isIconOnly
                            className={pages ? 'bg-default/20' : ''}
                            radius='full'
                            size='sm'
                            startContent={
                                pages ? <ArrowLeft size={18} /> : <SearchIcon size={18} />
                            }
                            variant={pages ? 'flat' : 'light'}
                            onPress={() => {
                                if (isPageMode()) {
                                    popPage()
                                    bounce(ref)
                                }
                            }}
                        />
                    }
                    value={query}
                    onFocus={() => setOpen(true)}
                    onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                            bounce(ref)
                        }

                        if (isPageMode() || query.length) {
                            return
                        }

                        if (e.key === 'Backspace') {
                            e.preventDefault()
                            popPage()
                            bounce(ref)
                        }
                    }}
                    onValueChange={(value) => {
                        setQuery(value)
                    }}
                />
            </Command>
        </AnimatedSearch>
    )
}
